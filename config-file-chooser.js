(() => {
  ...
  function addChooser(inputId, fileType, key, accept) {
    ...
    const wrapper = document.createElement('div'); wrapper.className = 'config-file-chooser-row';
    ...
    hidden.addEventListener('change', () => {
      const file = hidden.files?.[0]; if (!file) return;
      selected[fileType].set(String(key), file);
      if (window.phase2PackageFiles?.[fileType]) window.phase2PackageFiles[fileType].set(String(key), file);
      input.value = file.name; refresh();
    });
    wrapper.appendChild(button); wrapper.appendChild(hidden); wrapper.appendChild(status); refresh();
  }

  function watchModal() {
    const body = $('configModalBody'); if (!body || body.dataset.fileChooserObserver) return;
    ...
    const observer = new MutationObserver(() => {
      ...
      if (title.includes('Image Configuration')) addChooser('cfgImageFileName', 'image', index, '.png,.bmp,.jpg,.jpeg,.json');
      else if (title.includes('PFX Configuration')) addChooser('cfgPfxFileName', 'pfx', 0, '.p12,.pfx');
      else if (title.includes('Banner Configuration')) addChooser('cfgBannerName', 'banner', index, '.zip');
    });
