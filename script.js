...
function resetForm() {
  ...
  $('deleteTypeSection').classList.add('hidden');
...
function openModal(index = -1) {
  ...
  if (index >= 0) {
    ...
    $('deleteType').value = String(app.deleteType);
    $('tids').value = app.isDelete ? app.deleteTids.join('\n') : app.tids.join('\n');
...
function updateAutoInstallControl(index) {
  const dependency = isDependencyOfAnotherApp(index);
  $('autoInstall').disabled = dependency;
  if (dependency) $('autoInstall').checked = false;
}

function updateModeVisibility() {
  const isDelete = $('isDelete').checked;
  $('availabilitySection').classList.toggle('hidden', isDelete);
  $('deleteTypeSection').classList.toggle('hidden', !isDelete);
  const type = Number(isDelete ? $('deleteType').value : $('availabilityType').value);
  $('tidSection').classList.toggle('hidden', type !== 1);
}
...
function saveApp() {
  ...
  const isDelete = $('isDelete').checked;
  const selectedType = Number(isDelete ? $('deleteType').value : $('availabilityType').value);
  const enteredTids = $('tids').value.split(/\r?\n/).map(v => v.trim()).filter(Boolean);
  ...
  const app = normalizeApp({
    ...
    availabilityType: isDelete ? 0 : selectedType,
    tids: !isDelete && selectedType === 1 ? enteredTids : [],
    isDelete,
    deleteType: isDelete ? selectedType : 0,
    deleteTids: isDelete && selectedType === 1 ? enteredTids : [],
  ...
  if (index >= 0) state.appsConfig[index] = app; else state.appsConfig.push(app);
  enforceDependencyAutoInstall();
  renderApps(); refreshPreview(); closeModal(); toast(index >= 0 ? 'Application updated' : 'Application added');
}
...
function downloadJson() {
  ...
  link.href = url; link.download = 'appstore-config.json'; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url); toast('JSON downloaded');
}

createChecks($('modelOptions'), MODEL_OPTIONS, 'model');
createChecks($('androidOptions'), ANDROID_OPTIONS, 'android');
renderApps(); refreshPreview();
...
$('isDelete').addEventListener('change', updateModeVisibility);
$('availabilityType').addEventListener('change', updateModeVisibility);
$('deleteType').addEventListener('change', updateModeVisibility);
...
$('newBtn').addEventListener('click', () => {
  state.appsConfig = []; state.whiteListPackageName = ''; state.imageConfig = { config: [], timeStamp: '' }; state.pfxConfig = { pfxFileName: '', timeStamp: '' }; state.bannerConfig = [];
  state.supportConfig = { timeStamp: '', helpLine: '', preAuth: { dateExceededMessage: '', amountLimitMessage: '', completionReminderMessage: '' } };
  $('runBackground').checked = false; $('confirmation').checked = false; $('showError').checked = false; $('wifiOnly').checked = false;
  renderApps(); refreshPreview(); toast('New configuration created');
});
$('appsContainer').addEventListener('click', e => {
  ...
  if (action === 'clone') { const clone = structuredClone(state.appsConfig[index]); clone.title = `${clone.title} Copy`; state.appsConfig.splice(index + 1, 0, clone); enforceDependencyAutoInstall(); renderApps(); refreshPreview(); toast('Application cloned'); }
  if (action === 'delete') { if (confirm(`Delete ${state.appsConfig[index].title || state.appsConfig[index].appName}?`)) { state.appsConfig.splice(index, 1); enforceDependencyAutoInstall(); renderApps(); refreshPreview(); toast('Application deleted'); } }
});
