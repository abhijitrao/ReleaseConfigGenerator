(() => {
  ...
  function makeZip(entries) {
    ...
    entries.forEach(entry => {
      const name = encoder.encode(entry.name), data = entry.data instanceof Uint8Array ? entry.data : new Uint8Array(entry.data), crc = crc32(data);
      const localHeader = new Uint8Array([...u32(0x04034b50), ...u16(20), ...u16(0x0800), ...u16(0), ...u16(dt.time), ...u16(dt.date), ...u32(crc), ...u32(data.length), ...u32(data.length), ...u16(name.length), ...u16(0), ...name]);
      localParts.push(localHeader, data);
      const centralHeader = new Uint8Array([...u32(0x02014b50), ...u16(20), ...u16(20), ...u16(0x0800), ...u16(0), ...u16(dt.time), ...u16(dt.date), ...u32(crc), ...u32(data.length), ...u32(data.length), ...u16(name.length), ...u16(0), ...u16(0), ...u16(0), ...u16(0), ...u32(0), ...u32(offset), ...name]);
      centralParts.push(centralHeader); offset += localHeader.length + data.length;
    });
    const centralSize = centralParts.reduce((n, p) => n + p.length, 0), centralOffset = offset;
  ...
  function ensureModal() {
    ...
    $('closePackageBuilder').addEventListener('click', close); $('cancelPackageBuilder').addEventListener('click', close); $('buildPackageBtn').addEventListener('click', buildPackage);
  }
  ...
  function getConfigFiles() {
    ...
    const banners = Array.isArray(state.bannerConfig) ? state.bannerConfig : [];
    const pfx = state.pfxConfig?.pfxFileName ? [state.pfxConfig] : [];
    return { image, banners, pfx };
  }

  function renderFileRows() {
    const { image, banners, pfx } = getConfigFiles();
    const sections = [];
    const row = (type, key, label, folder) => {
      const file = selectedFiles[type].get(key);
      return `<div class="package-config-file-row"><div class="package-config-file-main"><b>${esc(label)}</b><span>${esc(folder)}/</span></div><div class="package-config-file-action"><span class="package-file-status ${file ? 'ready' : 'missing'}">${file ? `✓ ${esc(file.name)}` : 'Not selected'}</span><button type="button" class="secondary-btn" data-choose-package-file="${type}" data-key="${encodeURIComponent(key)}">Choose File</button><input type="file" hidden data-package-file-input="${type}" data-key="${encodeURIComponent(key)}"></div></div>`;
    };
    ...
    document.querySelectorAll('[data-choose-package-file]').forEach(button => button.addEventListener('click', () => {
      const input = button.parentElement.querySelector('input[data-package-file-input]'); if (input) input.click();
    }));
    document.querySelectorAll('[data-package-file-input]').forEach(input => input.addEventListener('change', e => {
      const file = e.target.files?.[0]; if (!file) return;
      const type = input.dataset.packageFileInput, key = decodeURIComponent(input.dataset.key); selectedFiles[type].set(key, file); renderFileRows();
    }));
  }

  function open() {
    ensureModal();
    const apps = getApplications(), missing = apps.filter(x => !x.file);
    ...
    $('packageBuilderError').classList.add('hidden'); renderFileRows(); modal.classList.remove('hidden');
  }

  function getConfigFileErrors() {
    const errors = [], { image, banners, pfx } = getConfigFiles();
    image.forEach((item,i) => { if (!selectedFiles.image.has(String(i))) errors.push(`Image Configuration: ${item.imageFileName || `image ${i+1}`} is not selected.`); });
    if (pfx.length && !selectedFiles.pfx.has('0')) errors.push(`PFX Configuration: ${pfx[0].pfxFileName} is not selected.`);
    banners.forEach((item,i) => { if (!selectedFiles.banner.has(String(i))) errors.push(`Banner Configuration: ${item.bannerName || `banner ${i+1}`} is not selected.`); });
    return errors;
  }

  async function buildPackage() {
    const apps = getApplications(), missing = apps.filter(x => !x.file), configErrors = getConfigFileErrors(), error = $('packageBuilderError');
    ...
    if (configErrors.length) { error.innerHTML = configErrors.map(esc).join('<br>'); error.classList.remove('hidden'); return; }
    ...
    try {
      ...
      const { image, banners, pfx } = getConfigFiles();
      for (let i=0;i<image.length;i++) entries.push({ name:`imageConfig/${image[i].imageFileName}`, data:new Uint8Array(await selectedFiles.image.get(String(i)).arrayBuffer()) });
      if (pfx.length) entries.push({ name:`pfxConfig/${pfx[0].pfxFileName}`, data:new Uint8Array(await selectedFiles.pfx.get('0').arrayBuffer()) });
      for (let i=0;i<banners.length;i++) entries.push({ name:`bannerConfig/${banners[i].bannerName}`, data:new Uint8Array(await selectedFiles.banner.get(String(i)).arrayBuffer()) });
    ...
    } catch (e) { error.textContent = e.message || 'Unable to build AppStore package.'; error.classList.remove('hidden'); }
    finally { button.disabled = false; button.textContent = 'Build & Download ZIP'; }
  }

  function close() { if (modal) modal.classList.add('hidden'); }
  function init() {
    if ($('buildPackageBtnTop')) return; const toolbar = document.querySelector('.toolbar'); if (!toolbar) return;
    buildButton = document.createElement('button'); buildButton.id = 'buildPackageBtnTop'; buildButton.type = 'button'; buildButton.textContent = 'Build Package'; buildButton.className = 'secondary-btn'; toolbar.insertBefore(buildButton, $('downloadBtn')); buildButton.addEventListener('click', open); ensureModal(); modal.addEventListener('click', e => { if (e.target === modal) return; });
  }
