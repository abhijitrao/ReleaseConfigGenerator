(() => {
  ...
  function renderApplicationRows() {
    ...
    document.querySelectorAll('[data-apk-input]').forEach(input => input.addEventListener('change', async e => {
      ...
      const index = Number(input.dataset.apkInput), app = state.appsConfig[index];
      try {
        if (typeof window.readApkMetadata === 'function') {
          const metadata = await window.readApkMetadata(file);
          if (metadata.packageName !== app.packageName) { showError(`APK package name mismatch for ${app.title || app.packageName}. Expected ${app.packageName}, found ${metadata.packageName}.`); return; }
          if (String(metadata.versionName || metadata.versionCode || '') !== String(app.appVersion || '')) { showError(`APK version mismatch for ${app.title || app.packageName}. Expected ${app.appVersion}, found ${metadata.versionName || metadata.versionCode}.`); return; }
        }
        selectedFiles.apk.set(String(index), file); renderBuilder();
      } catch (error) { showError(error.message || 'Unable to read selected APK.'); }
    }));
  }
