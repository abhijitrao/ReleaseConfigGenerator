(() => {
    ...
    while (p + 46 <= end && U32(bytes, p) === 0x02014b50) {
      ...
      if (method === 8) {
        ...
        try {
          ...
          throw new Error(`Unable to decompress ${entryName} from the APK.`);
        }
      }
  ...
  function decodeUtf16(bytes, offset, charCount, limit) {
    if (offset + charCount * 2 > limit) throw new Error('Invalid AndroidManifest.xml: UTF-16 string is truncated.');
    let out = '';
    for (let i = 0; i < charCount; i++) out += String.fromCharCode(U16(bytes, offset + i * 2));
    return out;
  }
  ...
  function parseStringPool(bytes, offset) {
    ...
    if (type !== 0x0001) throw new Error(`Invalid AndroidManifest.xml: string pool not found (chunk type 0x${type.toString(16)}).`);

    const headerSize = U16(bytes, offset + 2);
    const chunkSize = U32(bytes, offset + 4);
    const stringCount = U32(bytes, offset + 8);
    const styleCount = U32(bytes, offset + 12);
    const flags = U32(bytes, offset + 16);
    const stringsStart = U32(bytes, offset + 20);
    const stylesStart = U32(bytes, offset + 24);
    ...
    const offsetsEnd = offset + headerSize + stringCount * 4;
    if (offsetsEnd > offset + chunkSize || offsetsEnd > bytes.length) throw new Error('Invalid AndroidManifest.xml: string pool offsets are truncated.');
    if (stringsStart < headerSize || stringsStart >= chunkSize) throw new Error('Invalid AndroidManifest.xml: invalid string pool offset.');
    if (styleCount > 0 && stylesStart !== 0 && (stylesStart < headerSize || stylesStart >= chunkSize)) throw new Error('Invalid AndroidManifest.xml: invalid string pool styles offset.');

    const poolEnd = offset + chunkSize;
    const strings = [];
    const utf8 = (flags & 0x00000100) !== 0;
    const offsetsStart = offset + headerSize;

    const readLen8 = (pos) => {
      if (pos >= poolEnd) throw new Error('Invalid AndroidManifest.xml: UTF-8 string length is truncated.');
      let len = bytes[pos++];
      if (len & 0x80) {
        if (pos >= poolEnd) throw new Error('Invalid AndroidManifest.xml: UTF-8 string length is truncated.');
    ...
    const readLen16 = (pos) => {
      if (pos + 2 > poolEnd) throw new Error('Invalid AndroidManifest.xml: UTF-16 string length is truncated.');
    ...
    for (let i = 0; i < stringCount; i++) {
      ...
      const posStart = offset + stringsStart + rel;
      if (posStart < offset + stringsStart || posStart >= poolEnd) throw new Error('Invalid AndroidManifest.xml: string pool entry is truncated.');
      let pos = posStart;
      if (utf8) {
        const charLength = readLen8(pos); pos = charLength.pos;
        const byteLength = readLen8(pos); pos = byteLength.pos;
        if (pos + byteLength.len > poolEnd) throw new Error('Invalid AndroidManifest.xml: UTF-8 string is truncated.');
        strings.push(new TextDecoder('utf-8').decode(bytes.slice(pos, pos + byteLength.len)));
      } else {
        const length = readLen16(pos); pos = length.pos;
        strings.push(decodeUtf16(bytes, pos, length.len, poolEnd));
      }
    }
  ...
  function parseManifest(bytes) {
    ...
    const rootHeaderSize = U16(bytes, 2);
