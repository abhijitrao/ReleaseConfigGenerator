(() => {
  ...
  async function inflateRaw(data) { if (!('DecompressionStream' in window)) throw new Error('This browser does not support compressed ZIP entries.'); const stream = new Blob([data]).stream().pipeThrough(new DecompressionStream('deflate-raw')); return new Uint8Array(await new Response(stream).arrayBuffer()); }
  async function readZip(file) { const bytes = new Uint8Array(await file.arrayBuffer()), entries = []; let offset = 0; while (offset + 4 <= bytes.length) { const sig = signature(bytes, offset); if (sig === 0x04034b50) { const flags = u16(bytes, offset + 6), method = u16(bytes, offset + 8), compressedSize = u32(bytes, offset + 18), nameLength = u16(bytes, offset + 26), extraLength = u16(bytes, offset + 28), name = textDecoder.decode(bytes.slice(offset + 30, offset + 30 + nameLength)), dataStart = offset + 30 + nameLength + extraLength; if (flags & 0x08) throw new Error(`Unsupported ZIP data descriptor for: ${name}`); const compressed = bytes.slice(dataStart, dataStart + compressedSize); let data; if (method === 0) data = compressed; else if (method === 8) data = await inflateRaw(compressed); else throw new Error(`Unsupported ZIP compression for: ${name}`); entries.push({ name, data, directory: name.endsWith('/') }); offset = dataStart + compressedSize; } else if (sig === 0x02014b50 || sig === 0x06054b50) break; else throw new Error('Invalid AppStore ZIP package.'); } return entries; }
  ...
  async function importPackage(file) {
    ...
    // Preserve the imported outer ZIP filename as the default for the next Build Package operation.
