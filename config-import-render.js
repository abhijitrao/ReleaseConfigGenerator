(() => {
  ...
  function render(data) {
    ...
    const imageTimestamp = $('imageTimestampBadge');
    if (imageTimestamp) {
      imageTimestamp.textContent = image.timeStamp ? `Time Stamp: ${image.timeStamp}` : '';
      imageTimestamp.classList.toggle('hidden', !image.timeStamp);
    }
    const supportTimestamp = $('supportTimestampBadge');
    if (supportTimestamp) {
      supportTimestamp.textContent = support.timeStamp ? `Time Stamp: ${support.timeStamp}` : '';
      supportTimestamp.classList.toggle('hidden', !support.timeStamp);
    }
