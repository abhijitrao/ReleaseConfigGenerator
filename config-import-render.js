(() => {
  ...
  function render(data) {
    ...
    if (timestamp) {
      timestamp.textContent = image.timeStamp ? `Time Stamp: ${image.timeStamp}` : '';
      timestamp.classList.toggle('hidden', !image.timeStamp);
    }

    const imageList = $('imageConfigList');
    const images = Array.isArray(image.config) ? image.config : [];
    imageList.innerHTML = images.length ? images.map((item, i) => shell(
      ...
      [badge('Image', 'blue-soft'), badge(item.txnType || 'all')], 'image', i
    )).join('') : '<div class="empty">No image configuration added yet.</div>';

    const pfxList = $('pfxConfigList');
    pfxList.innerHTML = (pfx.pfxFileName || pfx.timeStamp) ? shell(
      ...
      [badge('PFX', 'blue-soft'), pfx.timeStamp ? badge(`Time Stamp ${pfx.timeStamp}`) : ''], 'pfx', 0
    ) : '<div class="empty">No PFX configuration added yet.</div>';
    if ($('addPfxBtn')) $('addPfxBtn').textContent = (pfx.pfxFileName || pfx.timeStamp) ? 'Edit PFX' : '+ Add PFX';

    const bannerList = $('bannerConfigList');
    bannerList.innerHTML = banners.length ? banners.map((item, i) => {
      const tidCount = Array.isArray(item.tids) ? item.tids.length : 0;
      const tidBased = Number(item.availabilityType) === 1;
      return shell(item.bannerName || 'Unnamed Banner',
        `<span>Banner ID: ${esc(item.bannerId || '-')}</span><span class="meta-separator">·</span><span>${tidBased ? `TID Based · ${tidCount} TID(s)` : 'All TIDs'}</span>`,
        [badge('Banner', 'blue-soft'), badge(tidBased ? 'TID Based' : 'All TIDs'), item.bannerId ? badge(`ID ${item.bannerId}`) : ''], 'banner', i);
    }).join('') : '<div class="empty">No banner configuration added yet.</div>';

    const supportList = $('supportConfigList');
    const messages = [
      ['Date Exceeded', support.preAuth.dateExceededMessage],
      ['Amount Limit', support.preAuth.amountLimitMessage],
      ['Completion Reminder', support.preAuth.completionReminderMessage]
    ];
    ...
    supportList.innerHTML = hasSupport ? shell(
      'Support & Pre-Auth',
      `<span>Help Line: ${esc(support.helpLine || '-')}</span><span class="meta-separator">·</span><span>Time Stamp: ${esc(support.timeStamp || '-')}</span>`,
      [badge('Support', 'blue-soft'), support.helpLine ? badge(`Help Line ${support.helpLine}`) : '', badge(`${messageCount} Pre-Auth Message${messageCount === 1 ? '' : 's'}`)],
      'support', 0,
      details ? `<div class="config-preview-line support-preview"><span class="config-preview-label">Pre-Auth</span><div class="support-message-list">${details}</div></div>` : ''
    ) : '<div class="empty">No support configuration added yet.</div>';
  }
