(() => {
  ...
  function renderPfx() {
    ...
    list.innerHTML = shell(pfx.pfxFileName || 'PFX Configuration', '<span>Client certificate</span>', [badge('PFX','blue-soft')], 'pfx', 0);
  ...
  function showModal(type,index=-1){ensureState();editingType=type;editingIndex=index;const body=$('configModalBody');let title='Add Configuration',subtitle='Enter configuration details.',html='';
    ...
    }else if(type==='pfx'){title=state.pfxConfig.pfxFileName?'Edit PFX Configuration':'Add PFX Configuration';subtitle='Configure the client certificate file.';html=`<div class="form-grid single-row"><div class="field"><label for="cfgPfxFileName">PFX File Name *</label><input id="cfgPfxFileName" value="${esc(state.pfxConfig.pfxFileName)}" placeholder="BHClient.p12"></div></div>`;
  ...
  function saveConfig(){
    ...
    }else if(editingType==='pfx'){const fileName=$('cfgPfxFileName').value.trim();if(!fileName)return toast('PFX File Name is required');state.pfxConfig={pfxFileName:fileName,timeStamp:state.pfxConfig.timeStamp||''};
    }else if(editingType==='banner'){const item={bannerName:$('cfgBannerName').value.trim(),bannerId:$('cfgBannerId').value.trim(),availabilityType:Number($('cfgBannerAvailability').value),tids:$('cfgBannerAvailability').value==='1'?$('cfgBannerTids').value.split(/\r?\n/).map(v=>v.trim()).filter(Boolean):[]};if(!item.bannerName)return toast('Banner Name is required');if(editingIndex>=0)state.bannerConfig[editingIndex]=item;else state.bannerConfig.push(item);
