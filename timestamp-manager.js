(() => {
  ...
  function markImported(rawJson){
    ...
    // Preserve whether the imported Support Config actually contained the key.
    // The UI normalizes a missing key to true, but adding that missing key must count as a change.
    if(rawJson && state.supportConfig){state.supportConfig.__chargeSlipUploadWasPresent = Object.prototype.hasOwnProperty.call(rawJson.supportConfig || {}, 'chargeSlipUpload');}
  ...
  document.addEventListener('change',event=>{
    if(event.target?.id==='fileInput'&&event.target.files?.length){
      ...
      reader.onload=()=>{try{markImported(JSON.parse(reader.result));}catch{markImported(null);}};
